...
function AbonneeCard({ abonnee }) {
  ...
  const deleteAbonnee = async () => {
    // const id = router.query.id
    try {
      await fetch(`http://localhost:3000/api/abonnees/${abonnee?._id}`, {
        method: 'Delete',
      })
